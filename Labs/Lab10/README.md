...
4. Looking at what your downloaded installer is quite interesting.  Go ahead, `cat` it or, better, pipe it to `more`.  Note there are **two** questions in the bullets for this part
   ...
   - Use the command on line 378 in your terminal to see if you can get the same result: `tail -n +589 "$THIS_PATH" | md5sum -` and replace `"$THIS_PATH"` with your installer's file name.  **Write the modified command**. (1 pt)
